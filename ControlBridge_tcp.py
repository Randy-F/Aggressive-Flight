...
class ControlBridge:
    ...
    def bridge_run(self):
        ...
        while True:
            ...
            tctimeClient,addr = self.tctime.accept()
            print("Connection from :",addr)

            while True:
                data = tctimeClient.recv(40) #.decode('utf-8', errors='ignore')
                print(data)
                if data is not None:
                    #建立链接后先起飞
                    if take_off == False:
                        ...
                        roll = 0.6
                        yawrate = 0.0
                        # self._cf.commander.send_setpoint(roll, pitch, yawrate, unpack_cmd[3])
